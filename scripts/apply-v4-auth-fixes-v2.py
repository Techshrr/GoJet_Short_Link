...
def replace_function(text,name,new_code):
    ...
    depth=0; end=None; in_string=False; raw=False; escape=False
...
func (s *server) register(w http.ResponseWriter, r *http.Request) {
	...
	if enabled, exists, _ := s.settings.Get(r.Context(), "registration.enabled"); exists && enabled == "false" {
		jsonResponse(w, 403, map[string]string{"error": "当前未开放注册"}); return
	}
	...
	if required, exists, _ := s.settings.Get(r.Context(), "registration.require_email_verification"); exists && required == "true" { requireVerification = true }
...
func (s *server) login(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { jsonResponse(w, 401, map[string]string{"error": "邮箱或密码错误"}); return }
...
if 'func (s *server) requestPasswordReset(' not in text:
    pos=text.find('func (s *server) resetPassword(')
...
text=main.read_text()
wrong='mux.HandleFunc("POST /api/auth/forgot-password", s.resetPassword)'
right='mux.HandleFunc("POST /api/auth/forgot-password", s.requestPasswordReset)'
if wrong in text: text=text.replace(wrong,right,1)
elif right not in text: raise SystemExit('forgot-password route pattern not found')
main.write_text(text)

admin=ROOT/'app/adminauth/service.go'
text=admin.read_text()
needle='\tif len(requested) == 0 && role != "custom" {\n\t\trequested = roleTemplates[role]\n\t}\n'
insert=needle+'\t// Every active administrator receives the read-only shell baseline.\n\trequested = append(requested, "platform.read")\n'
if 'requested = append(requested, "platform.read")' not in text:
    if needle not in text: raise SystemExit('normalizePermissions insertion point not found')
    text=text.replace(needle,insert,1)
admin.write_text(text)
print('V4 auth fixes v2 applied')
