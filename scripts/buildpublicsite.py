...
ACCOUNT_PAGES={'login','register','forgotpassword','resetpassword','verifyemail'}
...
FORBIDDEN=re.compile(r'Fresh Install|GoJet V4|不是静态演示数据|真实业务接口|Redis Worker|fileworker|analytics_events|RBAC 权限模型|V4 平台 API|后端业务闭环|可部署|product-hardening|hardening-release|/system-images/|SYSTEM_IMAGE_PATH|\?v=',re.I)
...
def build(output):
 ...
 shutil.copy2(SHARED/'gojetdesignsystem.css',output/'assets'/'gojetdesignsystem.css'); shutil.copy2(SHARED/'brandruntime.js',output/'assets'/'brandruntime.js')
 ...
 expected=set(GENERIC)|{'home','pricing','announcements','forgotpassword','login','privacy','register','reportabuse','resetpassword','status','terms','verifyemail'}|{f'products/{p}' for p in PRODUCTS}
