...
for path in INSTALL.md VERSION MANIFEST.sha256 install.sh install-host-nginx.sh install-native-lemp.sh launch-web-installer.sh \
  installer/index.php public/index.html public/app/index.html public/admin/index.html public/install/index.php \
  scripts/verify-published-release.sh scripts/native-installer-apply.sh scripts/install-docker.sh upgrade.sh rollback.sh \
  ...
  database/migrations app frontend services go.mod go.sum; do
