...
SOURCE=ROOT/'frontend'/'public-site'
PAGES=SOURCE/'pages'
ASSETS=SOURCE/'assets'
SHARED=ROOT/'frontend'/'shared'
DEFAULT_OUTPUT=ROOT/'build'/'public'
ACCOUNT_PAGES={'login','register','forgot-password','reset-password','verify-email'}
...
PRODUCTS={
'url-shortener':('短链接','SHORT LINKS','把冗长网址变成更容易分享、管理和复盘的短链接。',['自定义短码与跳转状态','链接发出后仍可维护目标','密码、有效期与访问规则'],['粘贴目标网址','设置短码与规则','分享并持续查看访问表现']),
'qr-code':('二维码','QR CODE','让线下物料通过同一条可维护短链接持续连接线上内容。',['二维码与短链接保持关联','多种尺寸与前景/背景颜色','单独统计二维码访问'],['选择正在使用的短链接','生成并下载二维码','更新链接目标而无需重印二维码']),
'analytics':('访问分析','ANALYTICS','用清晰的数据了解链接从哪里被打开、使用什么设备，以及访问变化。',['点击与独立访客','来源、地区、设备与浏览器','最近访问与二维码访问区分'],['创建并分享链接','等待真实访问发生','在工作区查看趋势与明细']),
'bio-pages':('个人主页','BIO PAGES','用一个长期入口整理品牌、社交账号、内容和活动链接。',['多种页面风格','链接排序与实时预览','公开地址长期保持稳定'],['创建主页并填写介绍','添加需要展示的入口','预览后发布并持续更新']),
'text-sharing':('文本分享','TEXT SHARING','快速分享普通文本、Markdown、代码片段和临时说明。',['密码保护与有效期','一次性读取规则','适合文本、Markdown 与代码'],['输入需要分享的内容','设置访问规则','复制分享地址']),
'file-sharing':('文件分享','FILE SHARING','在安全检查完成后生成可控的文件分享入口。',['上传后先执行安全检查','密码、有效期与下载次数','分享状态与下载次数可查看'],['选择一个文件上传','等待安全检查完成','复制分享地址并管理访问']),
'custom-domains':('自定义域名','CUSTOM DOMAINS','让短链接延续自己的品牌域名和信任感。',['DNS 所有权验证','HTTPS 状态检查','验证通过后用于短链接'],['添加需要使用的域名','按提示配置 DNS','验证通过后创建品牌短链']),
'smart-links':('智能链接','SMART LINKS','根据访问上下文把不同访客送达更合适的目标。',['按访问条件配置规则','保留统一公开入口','与访问分析结合复盘'],['创建统一短链接','配置访问规则','根据数据持续优化目标']),
'ab-testing':('A/B 测试','A/B TESTING','让同一入口在多个目标之间分配流量，用真实访问比较表现。',['多目标流量分配','保持一个公开短链接','结合访问结果持续调整'],['添加候选目标','配置流量比例','观察数据并选择更合适的方案']),
'qr-campaigns':('QR 营销活动','QR CAMPAIGNS','围绕活动批量组织二维码、短链接和访问表现。',['统一管理活动入口','线下物料与线上内容连接','访问结果集中复盘'],['建立活动与链接','生成所需二维码','上线后持续查看访问表现'])}

FORBIDDEN=re.compile(r'Fresh Install|GoJet V4|不是静态演示数据|真实业务接口|Redis Worker|file-worker|analytics_events|RBAC 权限模型|V4 平台 API|后端业务闭环|可部署|product-hardening|hardening-release|/system-images/|SYSTEM_IMAGE_PATH|\?v=',re.I)

def header(): return '<header class="siteHeader"><div class="container nav"><a class="logo" href="/">GoJet<i>.</i></a></div></header>'
def footer(): return '<footer><div class="container"><a class="logo" href="/">GoJet<i>.</i></a></div></footer><script src="/assets/app.js"></script>'
...
def generic_page(title,desc,cards,contact=False):
 ...
 extra='<section class="section alt"><div class="container split"><div><span class="eyebrow">已有账户</span><h2>优先从客户工单继续沟通</h2><p>工单会保留问题上下文、双方回复和当前处理状态，更适合持续跟进账户、账单和产品问题。</p><div class="actions"><a class="btn primary" href="/app/support">进入支持工单</a><a class="btn" href="/report-abuse/">滥用举报</a></div></div></div></section>' if contact else ''
...
def validate(route,html):
 ...
 if m: raise SystemExit(f'engineering copy or release-stage asset token leaked into {route}: {m.group(0)}')
 if route.startswith('products/'):
  for marker in('/assets/styles.css','/assets/app.js','siteHeader'):
   if marker not in html: raise SystemExit(f'product page {route} is missing canonical shell marker {marker}')

def write(output,route,html):
 validate(route,html)
 target=output/('index.html' if route=='home' else Path(route)/'index.html')
 target.parent.mkdir(parents=True,exist_ok=True); target.write_text(html,encoding='utf-8')

def build(output):
 if output.exists(): shutil.rmtree(output)
 output.mkdir(parents=True); shutil.copytree(ASSETS,output/'assets')
 shutil.copy2(SHARED/'gojet-design-system.css',output/'assets'/'gojet-design-system.css'); shutil.copy2(SHARED/'brand-runtime.js',output/'assets'/'brand-runtime.js')
 bad=list(PAGES.rglob('index.html'))
 if bad: raise SystemExit('canonical public source must not contain route index.html files')
 for src in sorted(PAGES.rglob('*.html')):
  rel=src.relative_to(PAGES); route='home' if rel==Path('home.html') else rel.with_suffix('').as_posix(); write(output,route,src.read_text(encoding='utf-8'))
 for route,(title,desc,cards) in GENERIC.items(): write(output,route,generic_page(title,desc,cards,route=='contact'))
 for slug,data in PRODUCTS.items(): write(output,f'products/{slug}',product_page(*data))
 write(output,'pricing',pricing_page())
 expected=set(GENERIC)|{'home','pricing','announcements','forgot-password','login','privacy','register','report-abuse','reset-password','status','terms','verify-email'}|{f'products/{p}' for p in PRODUCTS}
 built={'home' if p==output/'index.html' else p.parent.relative_to(output).as_posix() for p in output.rglob('index.html')}
 missing=sorted(expected-built)
 if missing: raise SystemExit('public build is missing routes: '+', '.join(missing))
 return len(built)

def main():
 p=argparse.ArgumentParser(); p.add_argument('--output',type=Path,default=DEFAULT_OUTPUT); a=p.parse_args(); count=build(a.output.resolve()); print(f'public site build ready: {a.output.resolve()} ({count} routes)')
