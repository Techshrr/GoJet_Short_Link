...
FORBIDDEN = re.compile(r'Fresh Install|GoJet V4|不是静态演示数据|真实业务接口|Redis Worker|file-worker|analytics_events|RBAC 权限模型|V4 平台 API|后端业务闭环|可部署|product-hardening|hardening-release|/system-images/|SYSTEM_IMAGE_PATH|\?v=rc12', re.I)
ACCOUNT_PAGES = {'login','register','forgot-password','reset-password','verify-email'}
PRODUCT_PAGES = {'ab-testing','analytics','bio-pages','custom-domains','file-sharing','qr-campaigns','qr-code','smart-links','text-sharing','url-shortener'}
...
def generic_page(title, desc, cards, contact=False):
    ...
    support=''
    if contact:
        support='<section class="section alt"><div class="container split"><div><span class="eyebrow">已有账户</span><h2>优先从客户工单继续沟通</h2><p>工单会保留问题上下文、双方回复和当前处理状态，更适合持续跟进账户、账单和产品问题。</p><div class="actions"><a class="btn primary" href="/app/support">进入支持工单</a><a class="btn" href="/report-abuse/">滥用举报</a></div></div><div class="uiPanel"><header><b>客户支持</b><small>可持续跟进</small></header><div class="row"><b>提交问题</b><small>选择支持部门</small><small>1</small></div><div class="row"><b>查看回复</b><small>同一会话</small><small>2</small></div><div class="row"><b>问题解决</b><small>关闭工单</small><small>3</small></div></div></div></section>'
...
def pricing_page():
    return '''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="GoJet 套餐覆盖个人使用、专业增长与团队协作；实际价格与配额以当前站点账户页面为准。"><title>套餐价格 · GoJet</title><link rel="stylesheet" href="/assets/styles.css"></head><body><header class="siteHeader"><div class="container nav"><a class="logo" href="/">GoJet<i>.</i></a></div></header><main><section class="hero"><div class="container"><span class="eyebrow">套餐价格</span><h1>从个人使用，到团队协作</h1><p>先从适合当前规模的套餐开始。实际价格、币种、资源配额和可用支付方式以登录后的账户页面与正式账单为准。</p><div class="actions"><a class="btn primary" href="/register">免费开始</a><a class="btn" href="/login">登录账户</a></div></div></section><section class="section"><div class="container"><div class="pricing"><article class="price"><span class="eyebrow">FREE</span><h3>Free</h3><b>适合个人试用与轻量分享</b><p>短链接、二维码和基础分享能力，适合先体验完整工作流程。</p><a class="btn" href="/register">免费开始</a></article><article class="price featured"><span class="eyebrow">PRO</span><h3>Pro</h3><b>适合持续运营和专业使用</b><p>更高资源配额、更多分析与品牌能力。具体价格以账户页面为准。</p><a class="btn primary" href="/register">创建账户</a></article><article class="price"><span class="eyebrow">BUSINESS</span><h3>Business</h3><b>适合多人团队与业务协作</b><p>面向团队工作区、成员管理和更高业务规模。具体方案以账户页面为准。</p><a class="btn" href="/contact/">了解更多</a></article></div></div></section><section class="section alt"><div class="container split"><div><span class="eyebrow">账单与汇率</span><h2>下单前确认实际币种和金额</h2><p>GoJet 统一以 USD 作为汇率基准。需要跨币种结算时，账单创建时会记录对应的结算金额和有效汇率，之后的参考汇率变化不会修改已经生成的账单。</p></div><div class="uiPanel"><header><b>账单信息</b><small>创建时确认</small></header><div class="row"><b>套餐</b><small>当前选择</small><small>明确</small></div><div class="row"><b>结算币种</b><small>账单显示</small><small>明确</small></div><div class="row"><b>应付金额</b><small>账单固定</small><small>明确</small></div></div></div></section><section class="cta"><div class="container"><div class="ctaBox"><h2>先创建账户，再选择适合的套餐</h2><p>账户页面会展示当前可用价格、配额和支付方式。</p><div class="actions" style="justify-content:center"><a class="btn" href="/register">免费开始</a></div></div></div></section></main><footer><div class="container"><a class="logo" href="/">GoJet<i>.</i></a></div></footer><script src="/assets/app.js"></script></body></html>'''


def output_path(rel):
    if rel == Path('home.html'):
...
def validate(route, html):
    ...
    if match:
        raise SystemExit(f'engineering copy or retired contract leaked into {route}: {match.group(0)}')
    if route.startswith('products/'):
        ...
        if 'product.css' in html or 'product-brand.css' in html:
            raise SystemExit(f'product page {route} returned to a secondary design system')
    if route not in ACCOUNT_PAGES and route != 'home' and '/assets/' not in html:
        raise SystemExit(f'public page {route} has no shared asset reference')


def write_page(output, route, html):
    validate(route, html)
...
def build(output):
    ...
    shutil.copy2(SHARED/'gojet-design-system.css', output/'assets'/'gojet-design-system.css')
    shutil.copy2(SHARED/'brand-runtime.js', output/'assets'/'brand-runtime.js')
    bad=list(PAGES.rglob('index.html'))
    if bad:
        raise SystemExit('canonical public source must not contain route index.html files: '+', '.join(map(str,bad)))
    for src in sorted(PAGES.rglob('*.html')):
        ...
        write_page(output, route, src.read_text(encoding='utf-8'))
    for route,(title,desc,cards) in GENERIC.items():
        write_page(output, route, generic_page(title,desc,cards,route=='contact'))
    write_page(output, 'pricing', pricing_page())
    expected=set(GENERIC)|{'home','pricing','announcements','forgot-password','login','privacy','register','report-abuse','reset-password','status','terms','verify-email'}|{f'products/{p}' for p in PRODUCT_PAGES}
...
def main():
    ...
    print(f'public site build ready: {args.output.resolve()} ({count} routes)')
