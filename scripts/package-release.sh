...
mkdir -p "$TARGET/bin" "$TARGET/installer" "$TARGET/database/migrations" \
  "$TARGET/deploy/nginx" "$TARGET/deploy/native" "$TARGET/deploy/docker" "$TARGET/scripts" "$TARGET/docs" \
  "$TARGET/public" "$TARGET/public/assets" "$TARGET/public/app" "$TARGET/public/admin" "$TARGET/public/install" \
...
# Runtime-only application payload. The production archive intentionally does
# not contain Go source, frontend canonical source, tests, audit notes or module
# build metadata. Docker images are assembled from the prebuilt binaries below.
...
cp "$ROOT/frontend/shared/brand-runtime.js" "$TARGET/public/assets/brand-runtime.js"
...
# Production Docker topology consumes only bin/, public/, resources/ and deploy/.
...
# Release packages serve all static product surfaces from public/. Source-tree
# installs may still use frontend/ paths, but release Nginx must never require
...
for config in "$TARGET/deploy/nginx/gojet-host.conf" "$TARGET/deploy/nginx/gojet-native.conf"; do
  sed -i 's#__GOJET_ROOT__/frontend/user-console/#__GOJET_ROOT__/public/app/#g; s#__GOJET_ROOT__/frontend/admin-console/#__GOJET_ROOT__/public/admin/#g; s#__GOJET_ROOT__/frontend/marketing-site#__GOJET_ROOT__/public#g' "$config"
done
...
cp "$ROOT/docs/deployment.zh-CN.md" "$ROOT/docs/architecture.md" "$ROOT/docs/object-storage.zh-CN.md" \
   "$ROOT/docs/operations-alerting.zh-CN.md" "$TARGET/docs/"
cp "$ROOT/install.sh" "$ROOT/install-host-nginx.sh" "$ROOT/install-native-lemp.sh" \
   "$ROOT/launch-web-installer.sh" "$ROOT/LICENSE" "$TARGET/"
...
# Release hygiene is a product requirement: engineering implementation labels,
# canonical source trees and obsolete asset namespaces must not ship.
...
if grep -R -n -E '/system-images/|SYSTEM_IMAGE_PATH|data/system/images|V4_PRODUCT_HARDENING|product-hardening|hardening-release' "$TARGET" --exclude=MANIFEST.sha256; then
