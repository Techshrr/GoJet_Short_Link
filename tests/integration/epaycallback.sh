...
# Configure the Epay-compatible gateway exactly through persisted settings. The
# provider implementation builds a redirect checkout locally, so no external
# network is required to prove our notification/settlement state machine.
...
notify=$(curl -sS -X POST -H 'Content-Type: application/x-www-form-urlencoded' --data "$form" -w $'\n%{http_code}' "$BASE/api/payments/epay/notify")
expect 200 "$notify" epay-notify | grep -Fxq success
...
# Provider retries are normal. The identical notification must remain idempotent
# and return the provider acknowledgement instead of reopening the transaction.
retry=$(curl -sS -X POST -H 'Content-Type: application/x-www-form-urlencoded' --data "$form" -w $'\n%{http_code}' "$BASE/api/payments/epay/notify")
expect 200 "$retry" epay-notify-retry | grep -Fxq success
...
printf 'GoJet Epay signed callback settlement acceptance: PASS (%s -> invoice %s -> pro)\n' "$order" "$invoice_id"
