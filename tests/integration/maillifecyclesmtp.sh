...
wait_type(){ local email=$1 type=$2; local count=0; for _ in $(seq 1 30); do count=$(mysqlq "SELECT COUNT(*) FROM mail_messages WHERE recipient='$email' AND message_type='$type';"); [[ "$count" -ge 1 ]] && return 0; sleep .2; done; cat /tmp/gojet/mailworker.log >&2; echo "mail type missing: $type" >&2; return 1; }
...
expect 202 "$(req POST /api/auth/forgotpassword "{\"email\":\"$user_email\"}")" password-reset-mail >/dev/null
expect 401 "$(req GET /api/me '' "$token")" forgotpassword-revokes-old-session >/dev/null
...
[[ "$failed" == 0 ]] || { cat /tmp/gojet/mailworker.log >&2; echo 'some lifecycle mail deliveries failed' >&2; exit 1; }
