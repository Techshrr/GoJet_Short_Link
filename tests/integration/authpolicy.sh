...
SOCIAL_PAYLOAD='{"auth.social.google.enabled":true,"auth.social.google.client_id":"google-client","auth.social.google.client_secret":"GoogleSecret-2026","auth.social.facebook.enabled":true,"auth.social.facebook.client_id":"facebook-client","auth.social.facebook.client_secret":"FacebookSecret-2026","auth.social.github.enabled":true,"auth.social.github.client_id":"github-client","auth.social.github.client_secret":"GitHubSecret-2026","auth.social.qq.enabled":true,"auth.social.qq.client_id":"qq-client","auth.social.qq.client_secret":"QQSecret-2026","auth.social.wechat.enabled":true,"auth.social.wechat.client_id":"wechat-client","auth.social.wechat.client_secret":"WechatSecret-2026","auth.social.rainbow.enabled":true,"auth.social.rainbow.client_id":"rainbow-client","auth.social.rainbow.client_secret":"RainbowSecret-2026","auth.social.rainbow.base_url":"https://u.cccyun.cc","auth.social.rainbow.login_type":"qq"}'
...
printf '%s' "$b"|python3 -c 'import json,sys; d=json.load(sys.stdin); assert [p["id"] for p in d["providers"]]==["google","facebook","github","qq","wechat","rainbow"]; assert "secret" not in str(d).lower()'
...
printf '%s' "$b"|python3 -c 'import json,sys; d=json.load(sys.stdin); s=d["socialauth"]; ids=["google","facebook","github","qq","wechat","rainbow"]; assert all(s[f"auth.social.{p}.client_secret"]=="********" for p in ids)'
...
b=$(expect 200 "$(req GET /api/admin/auth/providers '' "$ADMIN")" admin-provider-status);printf '%s' "$b"|python3 -c 'import json,sys; d=json.load(sys.stdin); assert len(d["providers"])==6; assert all(p["implemented"] for p in d["providers"]); assert all("callback_url" in p for p in d["providers"]); assert "client_secret" not in str(d).lower()'
