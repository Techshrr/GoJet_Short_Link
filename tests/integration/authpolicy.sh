...
echo '[1/17] login bootstrap administrator and mark SMTP health connected'
...
echo '[2/17] enable real registration policies'
...
echo '[3/17] blocked domain and password length really reject registration'
...
echo '[4/17] verified-email policy never returns or keeps an authenticated session'
...
echo '[5/17] correct password before verification still cannot create a live session'
...
echo '[6/17] verification link from real queued mail activates login'
...
echo '[7/17] login failure rate limit uses configured threshold'
...
echo '[8/17] disabling password recovery changes real endpoint behavior'
...
echo '[9/17] social identity schema enforces provider ownership and Google subject width'
...
[[ $(mysqlq "SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='user_social_identities' AND column_name='provider_subject' AND character_maximum_length=255;") == 1 ]] || { echo 'provider subject width does not cover Google sub' >&2; exit 1; }
...
echo '[10/17] disabled and incomplete social providers are never exposed publicly'
...
echo '[13/17] configured providers without an implemented adapter stay hidden'
expect 200 "$(req PUT /api/admin/settings/socialauth '{"auth.social.facebook.enabled":true,"auth.social.facebook.client_id":"not-live-yet","auth.social.facebook.client_secret":"not-live-yet-secret"}' "$ADMIN")" facebook-config >/dev/null
...
GOOGLE_SECRET='GoJetGoogleSecret-2026'
expect 200 "$(req PUT /api/admin/settings/socialauth "{\"auth.social.google.enabled\":true,\"auth.social.google.client_id\":\"gojet-google-client\",\"auth.social.google.client_secret\":\"$GOOGLE_SECRET\"}" "$ADMIN")" google-config >/dev/null
r=$(req GET /api/public/auth/providers); b=$(expect 200 "$r" google-visible)
printf '%s' "$b"|python3 -c 'import json,sys; d=json.load(sys.stdin); assert d["providers"] == [{"id":"google","label":"Google"}]'
[[ $(mysqlq "SELECT is_encrypted FROM system_settings WHERE setting_key='auth.social.google.client_secret';") == 1 ]] || { echo 'Google client secret is not encrypted' >&2; exit 1; }
GOOGLE_STORED=$(mysqlq "SELECT setting_value FROM system_settings WHERE setting_key='auth.social.google.client_secret';")
[[ "$GOOGLE_STORED" != *"$GOOGLE_SECRET"* ]] || { echo 'Google client secret stored in plaintext' >&2; exit 1; }
HEADERS_G=$(mktemp)
HEADERS_H=$(mktemp)
trap 'rm -f "$HEADERS_G" "$HEADERS_H"' EXIT
HTTP_CODE=$(curl -sS -D "$HEADERS_G" -o /dev/null -w '%{http_code}' "$BASE/api/public/auth/google/start?redirect=%2Fapp%2Fdashboard")
[[ "$HTTP_CODE" == 302 ]] || { echo "Google social start expected 302 got $HTTP_CODE" >&2; cat "$HEADERS_G" >&2; exit 1; }
LOCATION_G=$(awk 'BEGIN{IGNORECASE=1} /^Location:/{sub(/^[^:]*:[[:space:]]*/,""); sub(/\r$/,""); print; exit}' "$HEADERS_G")
STATE_G=$(python3 -c 'import sys,urllib.parse as u; q=u.parse_qs(u.urlparse(sys.argv[1]).query); print(q["state"][0])' "$LOCATION_G")
CHALLENGE_G=$(python3 -c 'import sys,urllib.parse as u; q=u.parse_qs(u.urlparse(sys.argv[1]).query); assert q["response_type"]==["code"]; assert q["code_challenge_method"]==["S256"]; assert set(q["scope"][0].split())=={"openid","profile","email"}; print(q["code_challenge"][0])' "$LOCATION_G")
python3 -c 'import sys,urllib.parse as u; p=u.urlparse(sys.argv[1]); assert p.scheme=="https" and p.netloc=="accounts.google.com" and p.path=="/o/oauth2/v2/auth"' "$LOCATION_G"
[[ ${#CHALLENGE_G} -eq 43 ]] || { echo 'Google PKCE challenge length invalid' >&2; exit 1; }
STATE_G_HASH=$(printf '%s' "$STATE_G"|sha256sum|awk '{print $1}')
[[ $(mysqlq "SELECT COUNT(*) FROM social_auth_attempts WHERE state_hash='$STATE_G_HASH' AND provider='google' AND consumed_at IS NULL AND expires_at>UTC_TIMESTAMP() AND CHAR_LENGTH(nonce_hash)=64 AND CHAR_LENGTH(pkce_verifier_hash)=64;") == 1 ]] || { echo 'Google social attempt was not persisted with browser binding hashes' >&2; exit 1; }
for cookie in gojet_social_state gojet_social_nonce gojet_social_pkce; do grep -i "^Set-Cookie: $cookie=" "$HEADERS_G"|grep -qi 'HttpOnly' || { echo "$cookie is not HttpOnly" >&2; exit 1; }; grep -i "^Set-Cookie: $cookie=" "$HEADERS_G"|grep -qi 'SameSite=Lax' || { echo "$cookie is not SameSite=Lax" >&2; exit 1; }; done

echo '[15/17] disabling Google blocks callback before consuming outstanding state'
expect 200 "$(req PUT /api/admin/settings/socialauth '{"auth.social.google.enabled":false}' "$ADMIN")" google-disable-callback >/dev/null
expect 403 "$(req GET "/api/public/auth/google/callback?code=fake-code&state=$STATE_G")" disabled-google-callback >/dev/null
[[ $(mysqlq "SELECT COUNT(*) FROM social_auth_attempts WHERE state_hash='$STATE_G_HASH' AND consumed_at IS NULL;") == 1 ]] || { echo 'disabled Google callback consumed state unexpectedly' >&2; exit 1; }
...
HTTP_CODE=$(curl -sS -D "$HEADERS_H" -o /dev/null -w '%{http_code}' "$BASE/api/public/auth/github/start?redirect=%2Fapp%2Fdashboard")
[[ "$HTTP_CODE" == 302 ]] || { echo "GitHub social start expected 302 got $HTTP_CODE" >&2; cat "$HEADERS_H" >&2; exit 1; }
LOCATION_H=$(awk 'BEGIN{IGNORECASE=1} /^Location:/{sub(/^[^:]*:[[:space:]]*/,""); sub(/\r$/,""); print; exit}' "$HEADERS_H")
STATE_H=$(python3 -c 'import sys,urllib.parse as u; q=u.parse_qs(u.urlparse(sys.argv[1]).query); print(q["state"][0])' "$LOCATION_H")
CHALLENGE_H=$(python3 -c 'import sys,urllib.parse as u; q=u.parse_qs(u.urlparse(sys.argv[1]).query); assert q["code_challenge_method"]==["S256"]; assert q["scope"]==["user:email"]; print(q["code_challenge"][0])' "$LOCATION_H")
...
[[ ${#CHALLENGE_H} -eq 43 ]] || { echo 'GitHub PKCE challenge length invalid' >&2; exit 1; }
STATE_H_HASH=$(printf '%s' "$STATE_H"|sha256sum|awk '{print $1}')
[[ $(mysqlq "SELECT COUNT(*) FROM social_auth_attempts WHERE state_hash='$STATE_H_HASH' AND provider='github' AND consumed_at IS NULL AND expires_at>UTC_TIMESTAMP() AND CHAR_LENGTH(nonce_hash)=64 AND CHAR_LENGTH(pkce_verifier_hash)=64;") == 1 ]] || { echo 'GitHub social attempt was not persisted with browser binding hashes' >&2; exit 1; }
...
expect 403 "$(req GET "/api/public/auth/github/callback?code=fake-code&state=$STATE_H")" disabled-github-callback >/dev/null
[[ $(mysqlq "SELECT COUNT(*) FROM social_auth_attempts WHERE state_hash='$STATE_H_HASH' AND consumed_at IS NULL;") == 1 ]] || { echo 'disabled GitHub callback consumed state unexpectedly' >&2; exit 1; }
