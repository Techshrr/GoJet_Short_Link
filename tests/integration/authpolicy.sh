...
printf '%s' "$b"|python3 -c 'import json,sys; d=json.load(sys.stdin); assert [p["id"] for p in d["providers"]]==["google","facebook","github","qq","wechat","rainbow"]; r=next(p for p in d["providers"] if p["id"]=="rainbow"); assert r["label"]=="快捷登录"; assert [x["id"] for x in r["login_types"]]==["qq","wx","alipay"]; assert "secret" not in str(d).lower(); assert "appkey" not in str(d).lower()'
expect 403 "$(req GET '/api/public/auth/rainbow/start?redirect=%2Fapp%2Fdashboard&type=douyin')" rainbow-unexposed-type >/dev/null
...
printf '%s' "$b"|python3 -c 'import json,sys; d=json.load(sys.stdin); s=d["socialauth"]; ids=["google","facebook","github","qq","wechat","rainbow"]; assert all(s[f"auth.social.{p}.client_secret"]=="********" for p in ids); assert s["auth.social.rainbow.display_name"]=="快捷登录"; assert s["auth.social.rainbow.login_types"]==["qq","wx","alipay"]; assert "auth.social.rainbow.login_type" not in s'
...
echo '[13/20] admin provider status is configuration only and reflects Rainbow policy'
b=$(expect 200 "$(req GET /api/admin/auth/providers '' "$ADMIN")" admin-provider-status);printf '%s' "$b"|python3 -c 'import json,sys; d=json.load(sys.stdin); ps=d["providers"]; assert len(ps)==6; assert all(p["implemented"] for p in ps); assert all("callback_url" in p for p in ps if p["id"]!="rainbow"); r=next(p for p in ps if p["id"]=="rainbow"); assert "callback_url" not in r; assert r["display_name"]=="快捷登录"; assert [x["id"] for x in r["login_types"]]==["qq","wx","alipay"]; assert "client_secret" not in str(d).lower()'
