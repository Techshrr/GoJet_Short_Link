...
go build -o "$tmp/log-receiver" ./services/log-receiver/cmd/server
MYSQL_DSN="gojet:integration@tcp(127.0.0.1:${mysql_port})/gojet?parseTime=true&charset=utf8mb4" LOG_INGEST_TOKEN=integration-log-token LOG_RECEIVER_HTTP_ADDRESS="127.0.0.1:${receiver_port}" "$tmp/log-receiver" >"$tmp/receiver.log" 2>&1 & receiver_pid=$!
...
response="$(curl -fsS -X POST "http://127.0.0.1:${receiver_port}/v1/logs" -H 'Authorization: Bearer integration-log-token' -H 'Content-Type: application/x-ndjson' --data-binary $'{"timestamp":"2026-08-09T12:00:00Z","service":"platform-api","level":"info","event":"http.request","request_id":"acceptance-log-1234","status":200,"duration_ms":9}\n{"timestamp":"2026-08-09T12:00:01Z","service":"redirect-engine","level":"error","event":"redis.timeout","request_id":"acceptance-log-1234"}\n')"
