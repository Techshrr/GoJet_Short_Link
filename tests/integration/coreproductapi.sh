...
for i in {1..60}; do
  ...
  sleep 1
  [[ $i -lt 60 ]] || { echo 'platformapi did not become healthy' >&2; exit 1; }
done
...
expect_status 202 "$(request POST /api/auth/forgotpassword '{"email":"public@example.test"}')" 'password reset request' >/dev/null
