...
echo '[1/8] login bootstrap administrator and mark SMTP health connected'
...
echo '[2/8] enable real registration policies'
...
echo '[3/8] blocked domain and password length really reject registration'
...
echo '[4/8] verified-email policy never returns or keeps an authenticated session'
...
TOKEN=$(printf '%s' "$HTML"|python3 -c 'import html,re,sys,urllib.parse as u; s=html.unescape(sys.stdin.read()); m=re.search(r"/verify-email\?token=([0-9a-f]{64})",s); assert m,"verification token missing"; print(m.group(1))')
expect 200 "$(req POST /api/auth/verify-email "{\"token\":\"$TOKEN\"}")" verify-email >/dev/null
r=$(req POST /api/auth/login '{"email":"verify@example.test","password":"VerifyPassword!2026"}'); expect 200 "$r" verified-login >/dev/null

echo '[7/8] login failure rate limit uses configured threshold'
