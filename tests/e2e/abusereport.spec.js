...
test('public abuse report page submits through the real product route',async({page})=>{
  await page.goto(base+'/reportabuse?url=https%3A%2F%2Fgojet.cc%2Fbadlink');
  await expect(page.getByRole('heading',{name:'举报滥用链接'})).toBeVisible();
  await expect(page.getByLabel('被举报的 GoJet 短链接')).toHaveValue('https://gojet.cc/badlink');
  ...
  expect(payload.url).toBe('https://gojet.cc/badlink');
