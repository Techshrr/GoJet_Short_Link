...
function CreateBioForm({ workspaceId, canEdit, c }: { workspaceId: number; canEdit: boolean; c: Copy }) {
  ...
  return <form className="resource-create-form bio-create-form" onSubmit={submit} data-bio-create-form>
    ...
    <Field label={c("Public slug", "公开短码")} htmlFor="bio-create-slug" help={c("Optional. Leave blank to generate one automatically.", "可选；留空时自动生成。") }><Input id="bio-create-slug" value={slug} onChange={(event) => setSlug(event.target.value)} placeholder="ethan" /></Field>
