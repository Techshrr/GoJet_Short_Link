...
function CreateLinkForm({ workspaceId, domains, organization }: { workspaceId: number; domains: LinkDomainOption[]; organization: OrganizationSnapshot }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (created) => window.location.assign(`/app/links/${created.id}?workspace=${workspaceId}`)
  });
  ...
  return <form className="links-create-form" onSubmit={submit}>
    <div className="links-create-scroll">
      ...
      <Field label="Destination" htmlFor="create-destination" required help="完整的 HTTP(S) 目标地址。"><Input id="create-destination" type="url" placeholder="https://example.com/page" value={destination} onChange={(event) => setDestination(event.target.value)} required /></Field>
    ...
    <div className="links-sheet-footer"><span className="links-muted">创建会立即写入 MySQL 并同步 Redis redirect plane。</span><Button type="submit" loading={mutation.isPending} disabled={!selectedDomain || !destination.trim()}>Create link</Button></div>
  </form>;
}
...
export default function LinksPage() {
  ...
  const setNumericFilter = (key: "campaign" | "tag", raw: string) => setFilters((current) => {
    const next: LinkListFilters = { ...current, offset: 0 };
  ...
  return <Page className="links-page" data-p05-links-list>
    ...
    <section className="links-toolbar" aria-label="Links filters">
      ...
      <Select aria-label="Campaign filter" value={filters.campaign ?? ""} onChange={(event) => setNumericFilter("campaign", event.target.value)}><option value="">All campaigns</option>{org.campaigns.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select>
      <Select aria-label="Tag filter" value={filters.tag ?? ""} onChange={(event) => setNumericFilter("tag", event.target.value)}><option value="">All tags</option>{org.tags.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select>
      ...
      <details className="links-columns"><summary>Columns · {visibleColumns}</summary><div>{(Object.keys(columns) as Array<keyof typeof columns>).map((key) => <Checkbox key={key} label={key.charAt(0).toUpperCase() + key.slice(1)} checked={columns[key]} onCheckedChange={(checked) => setColumns((current) => ({ ...current, [key]: checked }))} />)}</div></details>
    </section>
