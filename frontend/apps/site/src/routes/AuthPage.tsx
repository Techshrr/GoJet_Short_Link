...
export default function AuthPage({ mode }: { mode: Mode }) {
  ...
  useEffect(() => {
    ...
    if (socialError) {
      setError(`OAuth sign-in failed (${socialError}).`);
    ...
    if (handoff) {
      history.replaceState(null, "", location.pathname + location.search);
      setBusy(true);
      api.post<AuthResult>("/api/public/auth/handoff", { code: handoff })
        .then((result) => {
          if (!active) return;
          if (result.two_factor_required && result.challenge) {
            setChallenge(result.challenge);
            setPostAuthRedirect(result.redirect || "/app");
            setSuccess("Primary provider verified. Complete two-step verification.");
          } else {
            location.assign(result.redirect || "/app");
          }
        })
        .catch((ex) => active && setError(errText(ex)))
    ...
    if (registration) {
      history.replaceState(null, "", location.pathname + location.search);
      setBusy(true);
      api.get<SocialInfo>(`/api/public/auth/social-registration?code=${encodeURIComponent(registration)}`)
        .then((info) => {
          if (!active) return;
  ...
  const title = useMemo(
    () => ({
      login: "Sign in to GoJet",
      register: "Create your GoJet account",
  ...
  const sendRegistrationCode = async () => {
    ...
    try {
      ...
      setSuccess("Verification code sent. It expires in 10 minutes.");
    } catch (ex) {
      setError(errText(ex));
    } finally {
      setCodeBusy(false);
    ...
    try {
      ...
      if (mode === "register") {
        ...
        if (socialRegistration) {
          ...
          const result = await api.post<AuthResult>("/api/public/auth/social-registration/complete", {
            code: socialRegistration.code,
            display_name: name,
            email,
        ...
        const result = await api.post<AuthResult>("/api/auth/register", { email, password, display_name: name });
        if (result.verification_required) {
          setSuccess(
            result.verification_queued === false
              ? "Account created. Verification mail is temporarily unavailable; retry from sign in later."
      ...
      if (mode === "forgot") {
        ...
        setSuccess("If that account exists, a reset link has been queued.");
        return;
      }
      if (mode === "reset") {
        if (password !== confirm) throw new Error("Passwords do not match.");
        await api.post("/api/auth/resetpassword", { token, password });
  ...
  const subtitle = challenge
    ...
    : socialRegistration
      ? `Finish registration with ${socialRegistration.label}.`
      : mode === "login"
        ? "Use your account credentials or an enabled provider."
        : mode === "register"
          ? "Start with a personal workspace. You can add a team later."
          : mode === "forgot"
            ? "We never reveal whether an email address exists."
            : "Complete the account security step below.";

  return (
    <AuthShell
      visualTitle="One secure account for every GoJet workspace."
      visualBody="Short links, domains, QR, files and analytics stay behind one server-side session boundary."
    >
      <form className="auth-card" data-p15-auth={mode} onSubmit={submit}>
        <div>
          <h1>{challenge ? "Two-step verification" : title}</h1>
          ...
          <>
            <div className="auth-providers">
              {providers.map((provider) => (
                <button type="button" key={provider.id} onClick={() => location.assign(providerStart(provider, mode))}>
                  {provider.label}
                </button>
              ))}
            </div>
        ...
        {challenge ? (
          <label>
            ...
            <input autoFocus inputMode="numeric" autoComplete="one-time-code" value={code} onChange={(event) => setCode(event.target.value)} required />
          </label>
        ) : (
          <>
            {mode === "register" ? (
              <label>
                Display name
                <input autoComplete="name" value={name} onChange={(event) => setName(event.target.value)} required maxLength={120} />
              </label>
            ) : null}
            ...
            {socialRegistration ? (
              <div className="auth-code-row">
                <label>
                  ...
                  <input inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={emailCode} onChange={(event) => setEmailCode(event.target.value)} />
                </label>
                <button type="button" disabled={codeBusy || !email.includes("@")} onClick={sendRegistrationCode}>
            ...
            {mode === "register" || mode === "reset" ? (
              <label>
                Confirm password
                <input type="password" autoComplete="new-password" minLength={10} value={confirm} onChange={(event) => setConfirm(event.target.value)} required />
              </label>
            ) : null}
