...
export default function OAuthPage() {
  ...
  const save = useMutation({ mutationFn: ({ provider, draft }: { provider: Provider; draft: Draft }) => {
    const prefix = `auth.social.${provider.id}.`;
    const body: Record<string, unknown> = { [prefix + "enabled"]: draft.enabled, [prefix + "client_id"]: draft.clientId };
    if (draft.secret.trim()) body[prefix + "client_secret"] = draft.secret;
    if (provider.id === "rainbow") { body[prefix + "base_url"] = draft.baseUrl; body[prefix + "display_name"] = draft.displayName; body[prefix + "login_types"] = draft.loginTypes.split(",").map((value) => value.trim()).filter(Boolean); }
    return api.put("/api/admin/settings/socialauth", body);
  }, onSuccess: async (_, variables) => { setSaved(variables.provider.id); await Promise.all([qc.invalidateQueries({ queryKey: ["p15-oauth-providers"] }), qc.invalidateQueries({ queryKey: ["p15-settings-center"] })]); } });
...
function ProviderCard({ provider, values, saving, saved, error, onSave, c }: { provider: Provider; values: Record<string, unknown>; saving: boolean; saved: boolean; error: string; onSave: (draft: Draft) => void; c: Copy }) {
  ...
  return <section className="oauth-card">
    <header><div><h2>{provider.label}</h2><span>{provider.id}</span></div><div className="oauth-badges"><Badge tone={provider.implemented ? "success" : "neutral"}>{provider.implemented ? c("Supported", "已支持") : c("Unavailable", "暂不可用")}</Badge><Badge tone={provider.visible ? "success" : provider.enabled ? "warning" : "neutral"}>{availability}</Badge></div></header>
    {provider.callback_url ? <div className="oauth-callback"><span>{c("Callback address", "回调地址")}</span><code>{provider.callback_url}</code></div> : null}
    <label className="oauth-toggle"><input type="checkbox" checked={draft.enabled} onChange={(event) => setDraft({ ...draft, enabled: event.target.checked })}/><span>{c("Enable this sign-in provider", "启用这个登录服务")}</span></label>
    <Field label={provider.id === "rainbow" ? "APPID / Client ID" : "Client ID"} htmlFor={`${provider.id}-client-id`}><Input id={`${provider.id}-client-id`} value={draft.clientId} onChange={(event) => setDraft({ ...draft, clientId: event.target.value })}/></Field>
    <Field label={provider.id === "rainbow" ? "APPKEY / Client Secret" : "Client Secret"} htmlFor={`${provider.id}-secret`} help={provider.configured ? c("A secret is already configured. Leave this field blank to keep it unchanged.", "已有密钥。此处留空即可保持原密钥不变。") : c("No complete provider credentials are configured yet.", "当前尚未配置完整的登录服务凭据。") }><Input id={`${provider.id}-secret`} type="password" autoComplete="new-password" value={draft.secret} placeholder={c("Enter a new secret only when changing it", "仅在需要修改时填写新密钥")} onChange={(event) => setDraft({ ...draft, secret: event.target.value })}/></Field>
    ...
    {error ? <Alert tone="danger" title={c("Provider settings were not saved", "登录服务保存失败")}>{error}</Alert> : saved ? <Alert tone="info" title={c("Provider settings saved", "登录服务已保存")}>{c("The latest configuration has been reloaded. Existing stored secrets remain hidden.", "最新配置已重新加载，已保存的密钥仍保持隐藏。")}</Alert> : null}
    <Button type="button" disabled={!provider.implemented} loading={saving} onClick={() => onSave(draft)}>{c(`Save ${provider.label}`, `保存 ${provider.label}`)}</Button>
  </section>;
}
