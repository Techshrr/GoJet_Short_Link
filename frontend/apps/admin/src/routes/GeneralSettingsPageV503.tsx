...
export default function GeneralSettingsPageV503() {
  ...
  useEffect(() => { if (!query.data) return; const basic = query.data.basic ?? {}; setForm(Object.fromEntries(basicKeys.map((key) => [key, value(basic[key])])) as Record<BasicKey, string>); setPrimaryColor(value(query.data.brand?.["brand.primary_color"])); }, [query.data]);
  const save = useMutation({ mutationFn: async () => { await api.put("/api/admin/settings/basic", form); if (primaryColor.trim()) await api.put("/api/admin/settings/brand", { "brand.primary_color": primaryColor.trim() }); }, onSuccess: async () => { await qc.invalidateQueries({ queryKey: ["admin", "v503", "general-settings"] }); } });
  const upload = useMutation({ mutationFn: async ({ asset, file }: { asset: "logo" | "favicon"; file: File }) => { const data = new FormData(); data.append("file", file); return api.request<{ url: string }>(`/api/admin/brand/${asset}`, { method: "POST", body: data }); }, onSuccess: async () => { await qc.invalidateQueries({ queryKey: ["admin", "v503", "general-settings"] }); } });
  const remove = useMutation({ mutationFn: (asset: "logo" | "favicon") => api.delete(`/api/admin/brand/${asset}`), onSuccess: async () => { await qc.invalidateQueries({ queryKey: ["admin", "v503", "general-settings"] }); } });
  ...
  const brand = query.data?.brand ?? {}; const logo = value(brand.logo); const favicon = value(brand.favicon); const fallback = text("Request failed. Please try again.", "请求失败，请稍后重试。");
  return <Page data-p17-admin="general-v503">
    ...
    {query.isPending ? <Spinner label={text("Loading settings", "正在读取设置")} /> : query.isError ? <ErrorState title={text("Unable to load settings", "无法读取设置")} description={query.error instanceof Error ? query.error.message : fallback} /> : <>
      <section className="p17-action-card"><h2>{text("Site identity", "站点信息")}</h2><div className="p17-form-grid">
        ...
        <Field label={text("Default language", "默认语言")} htmlFor="site-language"><Select id="site-language" value={form["site.language"] || "zh-CN"} onChange={(e) => set("site.language", e.target.value)}><option value="zh-CN">{text("Chinese (Simplified)", "简体中文")}</option><option value="en">{text("English", "英语")}</option></Select></Field>
        <Field label={text("Timezone", "时区")} htmlFor="site-timezone"><Input id="site-timezone" value={form["site.timezone"]} onChange={(e) => set("site.timezone", e.target.value)} placeholder={text("Asia/Singapore", "Asia/Singapore")} /></Field>
      </div><Field label={text("Tagline", "站点标语")} htmlFor="site-tagline"><Input id="site-tagline" value={form["site.tagline"]} onChange={(e) => set("site.tagline", e.target.value)} /></Field><Field label={text("Site description", "站点描述")} htmlFor="site-description"><Textarea id="site-description" rows={4} value={form["site.description"]} onChange={(e) => set("site.description", e.target.value)} /></Field></section>
      <section className="p17-action-card"><h2>{text("Contact and company", "联系与公司信息")}</h2><div className="p17-form-grid">
      ...
      </div><Field label={text("Company address", "公司地址")} htmlFor="site-company-address"><Textarea id="site-company-address" rows={3} value={form["site.company_address"]} onChange={(e) => set("site.company_address", e.target.value)} /></Field></section>
      <section className="p17-action-card"><h2>{text("Brand assets", "品牌图片")}</h2><p>{text("Upload the logo and browser icon used by the site. Files are validated by the server and stored as managed brand assets.", "上传网站使用的 Logo 和浏览器图标。文件会经过服务器校验并作为受管理的品牌资源保存。")}</p><div className="p17-two-column">
      ...
      </div><Field label={text("Primary brand color", "品牌主色")} htmlFor="brand-primary-color"><Input id="brand-primary-color" value={primaryColor} onChange={(e) => setPrimaryColor(e.target.value)} placeholder={primaryColorExample} /></Field>{upload.isError || remove.isError ? <Alert tone="danger" title={text("Brand asset action failed", "品牌图片操作失败")}>{(upload.error || remove.error) instanceof Error ? (upload.error || remove.error as Error).message : fallback}</Alert> : null}</section>
      {save.isError ? <Alert tone="danger" title={text("Settings were not saved", "设置保存失败")}>{save.error instanceof Error ? save.error.message : fallback}</Alert> : save.isSuccess ? <Alert tone="info" title={text("Settings saved", "设置已保存")}>{text("The updated site identity is now stored on the server.", "最新站点信息已经保存到服务器。")}</Alert> : null}<Button type="button" loading={save.isPending} onClick={() => save.mutate()}>{text("Save settings", "保存设置")}</Button>
    </>}
  </Page>;
}
