...
export function PlanForm({ plan, busy = false, error, onSubmit }: Props) {
  ...
  return <form className="commerce-plan-form" onSubmit={submit}>
    ...
    <div className="commerce-form-grid"><Field label="Name" htmlFor="plan-name" required><Input id="plan-name" value={name} onChange={(event) => setName(event.target.value)} required /></Field><Field label="Currency" htmlFor="plan-currency" required help="Use a three-letter ISO currency code; the server validates the value."><Input id="plan-currency" value={currency} onChange={(event) => setCurrency(event.target.value)} required /></Field><Field label="Monthly price · cents" htmlFor="plan-price" required><Input id="plan-price" type="number" min="0" value={price} onChange={(event) => setPrice(event.target.value)} required /></Field><Field label="Display order" htmlFor="plan-order" required><Input id="plan-order" type="number" min="0" value={displayOrder} onChange={(event) => setDisplayOrder(event.target.value)} required /></Field></div>
