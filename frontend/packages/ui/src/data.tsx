...
export function DataTable<TData extends RowData>({
  ...
  error,
  emptyTitle = "暂无数据",
  emptyDescription = "当前条件下没有可显示的记录。",
  selectable = false,
  getRowId,
  toolbar,
  ...
  return (
    <section className="gj-data-table-region" aria-label={label}>
      <div className="gj-data-table-toolbar">
          ...
          <thead>
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id}>
                {selectable ? (
                  <th className="gj-data-table-select">
                    <Checkbox
                      checked={table.getIsAllRowsSelected()}
                      onCheckedChange={(checked) => table.toggleAllRowsSelected(checked)}
                      label={<span className="sr-only">选择全部行</span>}
                    />
                  </th>
                ) : null}
                  ...
                  return (
                    <th key={header.id} scope="col">
                      {header.isPlaceholder ? null : header.column.getCanSort() ? (
                        <button type="button" className="gj-data-table-sort" onClick={header.column.getToggleSortingHandler()} aria-sort={sorted === "asc" ? "ascending" : sorted === "desc" ? "descending" : "none"}>
                          <span>{flexRender(header.column.columnDef.header, header.getContext())}</span><SortIndicator direction={sorted} />
                        </button>
            ...
            {loading ? (
              Array.from({ length: 4 }, (_, rowIndex) => (
                <tr key={`loading-${rowIndex}`} aria-hidden="true">
                  {Array.from({ length: visibleColumns }, (_, cellIndex) => <td key={cellIndex}><Skeleton className="gj-data-table-skeleton" /></td>)}
                </tr>
              ))
            ) : error ? (
              <tr><td colSpan={visibleColumns}><ErrorState title="表格加载失败" description={error} /></td></tr>
            ) : table.getRowModel().rows.length === 0 ? (
              <tr><td colSpan={visibleColumns}><EmptyState title={emptyTitle} description={emptyDescription} /></td></tr>
            ) : table.getRowModel().rows.map((row) => (
              <tr key={row.id} data-selected={row.getIsSelected() || undefined}>
                {selectable ? <td className="gj-data-table-select"><Checkbox checked={row.getIsSelected()} onCheckedChange={(checked) => row.toggleSelected(checked)} label={<span className="sr-only">选择此行</span>} /></td> : null}
