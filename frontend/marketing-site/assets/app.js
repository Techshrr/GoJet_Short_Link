...
function installShell(){
  ...
  document.querySelectorAll('[data-gojet-year]').forEach(x=>x.textContent=new Date().getFullYear());
  ...
  mobile?.addEventListener('click',e=>{e.stopPropagation();toggle()});
...
function applyPublicSettings(settings){
  ...
  document.querySelectorAll('[data-gojet-site-name]').forEach(x=>x.textContent=name);
  document.querySelectorAll('.logo').forEach(logo=>{
    if(settings['brand.logo_url']){
      const img=document.createElement('img');img.src=settings['brand.logo_url'];img.alt=name;img.style.cssText='display:block;width:auto;max-width:150px;max-height:38px';logo.replaceChildren(img);
    }else if(logo.textContent.trim().startsWith('GoJet')){
      logo.innerHTML=`${String(name).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}<i>.</i>`;
    }
  });
  ...
  document.querySelectorAll('[data-support-email]').forEach(node=>{
    if(support){node.textContent=support;if(node.tagName==='A')node.href='mailto:'+support}else{node.textContent='用户控制台工单';if(node.tagName==='A')node.href='/app/'}
  });
}

installShell();
fetch('/api/public/settings',{cache:'no-store'}).then(r=>r.ok?r.json():null).then(applyPublicSettings).catch(()=>{});
})();