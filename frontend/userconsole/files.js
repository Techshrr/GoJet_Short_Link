...
async function load(){
  ...
  if(!host)return [];
  try{
    const items=(await api(`/api/workspaces/${state.workspace}/fileshares`)).data||[];
...
function upload(file){
  ...
  xhr.open('POST',`/api/workspaces/${state.workspace}/fileshares`);
...
window.deleteFileShare=async id=>{
  ...
  if(!proceed)return;
  await api(`/api/workspaces/${state.workspace}/fileshares/${id}`,{method:'DELETE'});
  await load();
};
