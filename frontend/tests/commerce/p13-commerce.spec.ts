...
for (const viewport of viewports) {
  test(`P13 Admin Commerce · ${viewport.name}`, async ({ page }) => {
    ...
    await page.keyboard.press("Escape"); await page.goto("/admin/fx"); await expect(page.getByRole("heading", { name: "Exchange rates", exact: true })).toBeVisible(); const save = page.getByRole("button", { name: "Save exchange-rate settings", exact: true }); await expect(save).toBeDisabled(); await page.getByLabel("Reason for change").fill("P13 browser gate audited override"); await save.click(); expect(state.fxUpdates).toBe(1); await expect(page.getByText("GJ-202608-0091")).toBeVisible(); await noOverflow(page);
