...
async function installApiFixture(page: Page, options: FixtureOptions = {}): Promise<FixtureState> {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (method === "GET" && path === "/api/session") return json(route, { authenticated: true, identity: { id: 7, email: "owner@example.com", displayName: "P06 Owner", emailVerified: true }, csrfToken: "p06-csrf" });
