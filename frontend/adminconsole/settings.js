...
renderSettings=async function(){
 ...
 UQ('#content').innerHTML=pageHead('系统设置','管理站点信息、账户规则、短链接、支付方式和品牌视觉。')+`<div class="ah-settings-shell"><aside class="ah-settings-nav">${tab('basic','站点信息','名称、企业信息与联系方式')}${tab('seo','搜索与分享','网页标题与摘要')}${tab('registration','注册与账户','注册、验证与密码安全')}${tab('links','短链接','默认短码与跳转规则')}${tab('payments','支付方式','账单在线支付渠道')}${tab('brand','品牌与视觉','Logo、浏览器图标与主品牌色')}</aside><div class="ah-settings-main">${basicPane}${searchPane}${accountPane}${linkPane}${paymentPane}${brandPane}</div></div>`;
