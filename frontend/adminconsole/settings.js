...
function embeddedPane(id){return `<section class="ah-settings-pane hidden" data-ah-pane="${id}"><div data-ah-embed="${id}"></div></section>`}
...
const assetLabels={logo:'网站 Logo',favicon:'浏览器图标'};
const settingTabs=['basic','seo','registration','links','payments','mail','botprotection','brand'];
...
renderSettings=async function(){
 ...
 const mailPane=embeddedPane('mail');
 const botProtectionPane=embeddedPane('botprotection');
 const assets=['logo','favicon'];
 const brandPane=`<section class="ah-settings-pane hidden" data-ah-pane="brand"><div class="ah-pane-head"><div><h2>品牌与视觉</h2><p>网站 Logo 统一用于官网、控制台、公开分享页、邮件与账单；浏览器图标用于标签页与收藏夹。</p></div></div><form class="ah-brand-color" data-section="brand">${input('brand.primary_color','主品牌色',value(brand,'brand.primary_color','#16A66A'),'color')}<button class="btn primary">保存品牌色</button><span class="ah-save-result"></span></form><div class="ah-assets">${assets.map(a=>`<article class="ah-asset"><div class="ah-asset-preview">${brand[a]?`<img src="${UE(brand[a])}" alt="${UE(assetLabels[a])}">`:'<span>未设置</span>'}</div><div><b>${UE(assetLabels[a])}</b><small>${brand[a]?UE(brand[a]):'尚未上传'}</small><input type="file" accept="image/png,image/jpeg,image/webp,image/gif,image/x-icon" data-ah-file="${a}"><div class="row-actions"><button type="button" class="btn small" data-ah-upload="${a}">上传</button>${brand[a]?`<button type="button" class="btn small danger" data-ah-delete="${a}">删除</button>`:''}</div></div></article>`).join('')}</div></section>`;
 UQ('#content').innerHTML=pageHead('系统设置','管理站点、账户、短链接、支付、邮件、人机验证与品牌视觉。')+`<div class="ah-settings-shell"><aside class="ah-settings-nav">${tab('basic','站点信息','名称、企业信息与联系方式')}${tab('seo','搜索与分享','网页标题与摘要')}${tab('registration','注册与账户','注册、验证与密码安全')}${tab('links','短链接','默认短码与跳转规则')}${tab('payments','支付方式','账单在线支付渠道')}${tab('mail','邮件服务','SMTP、模板与投递状态')}${tab('botprotection','人机验证','Turnstile 与防机器人场景')}${tab('brand','品牌与视觉','Logo、浏览器图标与主品牌色')}</aside><div class="ah-settings-main">${basicPane}${searchPane}${accountPane}${linkPane}${paymentPane}${mailPane}${botProtectionPane}${brandPane}</div></div>`;
 UQA('[data-ah-tab]').forEach(b=>b.onclick=()=>void openUnifiedTab(b.dataset.ahTab));
 ...
 const requested=sessionStorage.getItem('gojet_settings_tab')||'basic';
 await openUnifiedTab(settingTabs.includes(requested)?requested:'basic');
};

async function openUnifiedTab(id){
 if(!settingTabs.includes(id))id='basic';
 sessionStorage.setItem('gojet_settings_tab',id);
 UQA('[data-ah-tab]').forEach(b=>b.classList.toggle('active',b.dataset.ahTab===id));
 UQA('[data-ah-pane]').forEach(p=>p.classList.toggle('hidden',p.dataset.ahPane!==id));
 const pane=UQ(`[data-ah-pane="${id}"]`);
 if(id==='mail'){
  const mount=UQ('[data-ah-embed="mail"]',pane);
  if(!mount||typeof window.renderMailSettings!=='function')return;
  try{await window.renderMailSettings(mount,{embedded:true})}catch(err){mount.innerHTML=`<div class="alert">${UE(err.message)}</div>`;fail(err.message)}
 }
 if(id==='botprotection'){
  const mount=UQ('[data-ah-embed="botprotection"]',pane);
  if(!mount||typeof window.renderBotProtectionSettings!=='function')return;
  try{await window.renderBotProtectionSettings(mount,{embedded:true})}catch(err){mount.innerHTML=`<div class="alert">${UE(err.message)}</div>`;fail(err.message)}
 }
}
