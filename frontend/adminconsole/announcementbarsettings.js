...
function deactivateAnnouncement(tab,pane){tab.classList.remove('active');pane.classList.add('hidden')}
async function install(){
  ...
  try{
    ...
    const tab=document.createElement('button');tab.type='button';tab.dataset.announcementbarTab='1';tab.dataset.ahTab='announcementbar';tab.innerHTML='<b>顶部飘窗</b><small>官网导航上方的优惠与重要事项</small>';
    const pane=document.createElement('form');pane.className='ah-settings-pane hidden';pane.dataset.announcementbarPane='1';pane.dataset.ahPane='announcementbar';
    pane.innerHTML=`<div class="ah-pane-head"><div><h2>顶部飘窗</h2><p>在官网导航栏上方显示一条全宽公告。内容、链接、展示时间和关闭方式均由这里控制。</p></div><button class="btn primary">保存设置</button></div><div class="ah-settings-grid">
      <label class="ah-switch full"><span><b>启用顶部飘窗</b><small>关闭后官网立即停止显示。</small></span><input name="announcementbar.enabled" type="checkbox" ${settings['announcementbar.enabled']?'checked':''}></label>
    ...
    tab.onclick=event=>{event.preventDefault();nav.querySelectorAll('button').forEach(node=>node.classList.remove('active'));main.querySelectorAll('.ah-settings-pane,[data-ah-pane]').forEach(node=>node.classList.add('hidden'));tab.classList.add('active');pane.classList.remove('hidden')};
    nav.addEventListener('click',event=>{const button=event.target.closest('button');if(button&&button!==tab)deactivateAnnouncement(tab,pane)},true);
