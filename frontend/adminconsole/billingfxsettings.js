...
function linesToManualRates(text){
  ...
  for(const raw of String(text||'').split(/\r?\n/)){
    ...
    if(!/^USD\/[A-Z]{3}$/.test(pair)||pair==='USD/USD'||!/^\d+(?:\.\d{1,12})?$/.test(rate)||Number(rate)<=0)throw new Error(`手动汇率必须以 USD 为基准，例如 USD/CNY=7.250000；当前无效：${line}`);
    out[pair]=rate;
  }
...
renderSettings=async function(){
  ...
  nav.insertAdjacentHTML('beforeend',`<button type="button" data-ah-tab="billing"><b>账单与汇率</b><small>USD 基准汇率与结算币种</small></button>`);
  ...
  main.insertAdjacentHTML('beforeend',`<form class="ah-settings-pane hidden" data-ah-pane="billing" data-section="billing">
    <div class="ah-pane-head"><div><h2>账单与汇率</h2><p>GoJet 统一以 USD 作为汇率基准。所有币种先归一到 USD，再换算到目标结算币种；账单创建时会冻结最终金额与有效汇率。</p></div><button class="btn primary">保存设置</button></div>
    <div class="ah-settings-grid">
      <label><span>汇率基准币种</span><input value="USD — 美元" disabled><small class="settingHelp">系统固定基准，不可修改。USD 自身汇率恒为 1。</small></label>
      ...
      <label><span>汇率来源</span><select name="billing.fx.provider"><option value="ecb" ${provider==='ecb'?'selected':''}>欧洲中央银行参考汇率（归一为 USD）</option><option value="manual" ${provider==='manual'?'selected':''}>手动 USD 基准汇率</option></select><small class="settingHelp">外部参考数据会先统一换算成“1 USD = X 目标币种”后再进入缓存。</small></label>
      <label><span>汇率调整（基点）</span><input name="billing.fx.markup_bps" type="number" min="-1000" max="5000" value="${markup}"><small class="settingHelp">100 个基点 = 1%。调整作用于最终源币种 → 结算币种的有效汇率。</small></label>
      <label><span>参考汇率缓存时间（小时）</span><input name="billing.fx.cache_hours" type="number" min="1" max="168" value="${cache}"><small class="settingHelp">缓存中只保存 USD/目标币种。无法取得可靠基准汇率时，不生成跨币种账单。</small></label>
      <label class="full"><span>手动 USD 基准汇率</span><textarea id="manualFxRates" rows="7" placeholder="USD/CNY=7.250000\nUSD/EUR=0.860000\nUSD/HKD=7.800000">${escFX(manualRatesToLines(billing['billing.fx.manual_rates']))}</textarea><small class="settingHelp">仅在选择“手动 USD 基准汇率”时使用。每行必须是 USD/币种，不接受 EUR/CNY、CNY/USD 等其他基准或反向货币对。</small></label>
