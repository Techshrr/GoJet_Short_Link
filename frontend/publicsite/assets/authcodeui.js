...
function storeAndRedirect(result){if(result?.token)localStorage.setItem('gojet_token',result.token);const target=new URLSearchParams(location.search).get('redirect');location.replace(target?.startsWith('/')&&!target.startsWith('//')?target:'/app/dashboard')}

policy().then(settings=>{
  if(page==='register'&&settings.registration_code_required){
    const emailField=form.querySelector('input[name="email"]')?.closest('.field');emailField?.insertAdjacentHTML('afterend',codeField('registerEmailCode'));
    const code=document.querySelector('#registerEmailCode'),send=document.querySelector('#registerEmailCodeSend');if(code)code.required=true;
    send?.addEventListener('click',()=>sendCode('register',send).catch(err=>show(err.message)));
    const submit=form.querySelector('button[type="submit"]');if(submit)submit.textContent='验证邮箱并创建账户';
    const lead=document.querySelector('.auth-card .lead');if(lead)lead.textContent='填写账户信息并验证邮箱，验证通过后即可直接进入 GoJet。';
    form.addEventListener('submit',async event=>{event.preventDefault();event.stopImmediatePropagation();const data=Object.fromEntries(new FormData(form));if(data.password!==data.password_confirmation)return show('两次输入的密码不一致');if(!/^\d{6}$/.test(String(data.email_code||'')))return show('请输入邮件中的 6 位验证码。');const button=form.querySelector('button[type="submit"]');button.disabled=true;button.textContent='正在验证并创建…';try{storeAndRedirect(await call('/api/public/register-email-code',{email:data.email,password:data.password,display_name:data.display_name,email_code:data.email_code}))}catch(err){show(err.message);button.disabled=false;button.textContent='验证邮箱并创建账户'}},true);
  }
  if(page==='login'&&settings.email_code_login_available){
    let mode='password';const passwordField=form.querySelector('input[name="password"]')?.closest('.field'),emailField=form.querySelector('input[name="email"]')?.closest('.field'),forgot=form.querySelector('.row'),submit=form.querySelector('button[type="submit"]');
    const tabs=document.createElement('div');tabs.className='auth-mode-tabs';tabs.innerHTML='<button type="button" class="active" data-login-mode="password">密码登录</button><button type="button" data-login-mode="code">验证码登录</button>';form.insertBefore(tabs,form.firstElementChild);emailField?.insertAdjacentHTML('afterend',codeField('loginEmailCode'));const codeFieldEl=document.querySelector('#loginEmailCodeField'),code=document.querySelector('#loginEmailCode'),send=document.querySelector('#loginEmailCodeSend');codeFieldEl?.classList.add('hidden');send?.addEventListener('click',()=>sendCode('login',send).catch(err=>show(err.message)));
    ...
    form.addEventListener('submit',async event=>{if(mode!=='code')return;event.preventDefault();event.stopImmediatePropagation();const data=Object.fromEntries(new FormData(form));if(!/^\d{6}$/.test(String(data.email_code||'')))return show('请输入邮件中的 6 位验证码。');submit.disabled=true;submit.textContent='正在验证…';try{storeAndRedirect(await call('/api/public/login-email-code',{email:data.email,email_code:data.email_code}))}catch(err){show(err.message);submit.disabled=false;submit.textContent='使用验证码登录'}},true);
  }
});
})();
