...
function assetVersion(){return 'hardening-2'}
...
const hardeningReady=Promise.all([
 loadStyle('/app/link-hardening.css'),loadStyle('/app/bio-hardening.css'),loadStyle('/app/domain-hardening.css'),
 loadScript('/app/link-hardening.js'),loadScript('/app/bio-hardening.js'),loadScript('/app/domain-hardening.js')
]);
...
document.addEventListener('click',e=>{const top=e.target.closest('#createButton');if(top){e.preventDefault();e.stopImmediatePropagation();hardeningReady.then(()=>window.openLinkHardening?.());return}const b=e.target.closest('[data-console-view]');if(!b)return;e.preventDefault();e.stopImmediatePropagation();show(b.dataset.consoleView,true)},true);
...
async function renderGlobalAnalytics(){
 ...
 const clicks=overview.total_clicks??overview.clicks??rows.reduce((n,x)=>n+Number(x.clicks||x.Clicks||0),0),visitors=overview.unique_visitors??overview.visitors??rows.reduce((n,x)=>n+Number(x.unique_visitors||x.visitors||x.Visitors||0),0),active=overview.active_links??rows.filter(x=>(x.status||x.Status)==='active').length;
