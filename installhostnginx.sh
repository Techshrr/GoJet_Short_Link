...
export GOJET_COMPOSE_OVERRIDE="$ROOT/deploy/compose.hostnginx.yaml"
. "$ROOT/scripts/lib.sh"
...
[ "$(id -u)" -eq 0 ] || die "host Nginx installation must run as root (use sudo ./installhostnginx.sh)"
...
sed "s|__GOJET_ROOT__|$ROOT|g" "$ROOT/deploy/nginx/gojethost.conf" > "$rendered"
