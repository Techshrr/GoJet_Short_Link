...
type CreatePolicy struct {
	...
	DefaultRedirectStatus, CodeLength, DefaultExpiryDays int
	DefaultClickLimit                                    int64
	ReservedCodes, BlockedKeywords                       []string
	ForceHTTPS                                           bool
}
...
func (s *Service) Create(ctx context.Context, userID, workspaceID int64, l Link) (Link, error) {
	return s.CreateWithPolicy(ctx, userID, workspaceID, l, CreatePolicy{})
}

func (s *Service) CreateWithPolicy(ctx context.Context, userID, workspaceID int64, l Link, policy CreatePolicy) (Link, error) {
...
func randomCodeFrom(length int, alphabet string) string {
	if alphabet == "" {
		alphabet = "abcdefghijkmnopqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ23456789"
	}
	chars := []byte(alphabet)
	if len(chars) < 2 {
		chars = []byte("abcdefghijkmnopqrstuvwxyz23456789")
	}
	b := make([]byte, length)
...
func applyCreatePolicy(l Link, policy CreatePolicy, now time.Time) (Link, error) {
	if l.Domain == "" {
		l.Domain = strings.TrimSpace(policy.DefaultDomain)
	}
	...
	if policy.ForceHTTPS {
		...
		if err != nil || u.Scheme != "https" {
			return Link{}, errors.New("系统策略要求目标地址使用 HTTPS")
		}
	}
	if l.Code == "" {
		...
		l.Code = randomCodeFrom(length, policy.AllowedCharacters)
	}
	if policy.AllowedCharacters != "" {
		for _, char := range l.Code {
	...
	for _, reserved := range policy.ReservedCodes {
		if lowerCode == strings.ToLower(strings.TrimSpace(reserved)) {
			return Link{}, errors.New("短码已被系统保留")
		}
	}
	for _, blocked := range policy.BlockedKeywords {
		blocked = strings.ToLower(strings.TrimSpace(blocked))
		if blocked != "" && strings.Contains(lowerCode, blocked) {
			return Link{}, errors.New("短码包含系统禁用关键词")
		}
	}
	return l, nil
}
