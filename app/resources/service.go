...
func NewFileWorker(db *sql.DB, path string) *Service {
	return &Service{db: db, filePath: path, files: objectstorage.Filesystem{Root: path}}
}

func (s *Service) WithFileStore(store objectstorage.Store) *Service { s.files = store; return s }
