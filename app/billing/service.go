...
var billingCycles = map[string]cycleDefinition{
	"monthly":    {Name: "monthly", Months: 1},
	"quarterly":  {Name: "quarterly", Months: 3},
	"semiannual": {Name: "semiannual", Months: 6},
	"annual":     {Name: "annual", Months: 12},
}

func normalizeBillingCycle(raw string) (cycleDefinition, error) {
	cycle := strings.ToLower(strings.TrimSpace(raw))
	if cycle == "" {
		cycle = "monthly"
	}
	definition, ok := billingCycles[cycle]
	if !ok {
		return cycleDefinition{}, errors.New("账单周期无效")
	}
...
const invoiceSelect = `i.id,i.invoice_number,i.workspace_id,i.plan_id,i.requested_by,p.name,p.code,i.invoice_type,i.billing_cycle,i.period_months,i.source_amount_cents,i.source_currency,i.amount_cents,i.currency,CAST(i.fx_rate AS CHAR),i.fx_provider,i.fx_markup_bps,i.fx_quoted_at,i.period_days,i.status,i.due_at,i.paid_at,i.admin_note,i.created_at`

func scanInvoice(scanner interface{ Scan(...any) error }, i *Invoice) error {
	var due, created time.Time
	var paid, quoted sql.NullTime
	if err := scanner.Scan(&i.ID, &i.InvoiceNumber, &i.WorkspaceID, &i.PlanID, &i.RequestedBy, &i.PlanName, &i.PlanCode, &i.InvoiceType, &i.BillingCycle, &i.PeriodMonths, &i.SourceAmountCents, &i.SourceCurrency, &i.AmountCents, &i.Currency, &i.FXRate, &i.FXProvider, &i.FXMarkupBPS, &quoted, &i.PeriodDays, &i.Status, &due, &paid, &i.AdminNote, &created); err != nil {
		return err
	}
	...
	if quoted.Valid {
		i.FXQuotedAt = quoted.Time.UTC().Format(time.RFC3339)
	}
	return nil
}
...
func (s *Service) Request(ctx context.Context, userID, workspaceID int64, planCode, kind, cycle string) (Invoice, error) {
	if err := s.Reconcile(ctx); err != nil {
		return Invoice{}, err
	}
	...
	cycleDef, err := normalizeBillingCycle(cycle)
	if err != nil {
		return Invoice{}, err
	}
	var role string
	...
	if plan.MonthlyPriceCents > 0 && int64(cycleDef.Months) > (1<<63-1)/plan.MonthlyPriceCents {
		return Invoice{}, errors.New("套餐周期金额超出允许范围")
	}
	sourceTotal := plan.MonthlyPriceCents * int64(cycleDef.Months)
	snapshot := FXSnapshot{SourceAmount: sourceTotal, SourceCurrency: strings.ToUpper(plan.Currency), Amount: sourceTotal, Currency: strings.ToUpper(plan.Currency), Rate: "1.000000000000", Provider: "identity", QuotedAt: time.Now().UTC()}
	if s.fx != nil {
		snapshot, err = s.fx.QuoteSettlement(ctx, plan.Currency, sourceTotal)
		if err != nil {
			return Invoice{}, err
		}
	}
	...
	legacyDays := cycleDef.Months * 30
	result, err := tx.ExecContext(ctx, `INSERT INTO billing_invoices(invoice_number,workspace_id,plan_id,requested_by,invoice_type,billing_cycle,period_months,source_amount_cents,source_currency,amount_cents,currency,fx_rate,fx_provider,fx_markup_bps,fx_quoted_at,period_days,due_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, number, workspaceID, plan.ID, userID, kind, cycleDef.Name, cycleDef.Months, snapshot.SourceAmount, snapshot.SourceCurrency, snapshot.Amount, snapshot.Currency, snapshot.Rate, snapshot.Provider, snapshot.MarkupBPS, snapshot.QuotedAt, legacyDays, due)
	if err != nil {
		return Invoice{}, err
	}
	id, _ := result.LastInsertId()
	_, err = tx.ExecContext(ctx, `INSERT INTO subscription_events(workspace_id,invoice_id,actor_type,actor_id,event_type,from_plan_id,to_plan_id,metadata) VALUES(?,?,'user',?,'invoice.requested',?,?,JSON_OBJECT('invoice_number',?,'billing_cycle',?,'period_months',?,'source_amount',?,'source_currency',?,'settlement_amount',?,'settlement_currency',?,'fx_rate',?,'fx_provider',?))`, workspaceID, id, userID, currentPlan, plan.ID, number, cycleDef.Name, cycleDef.Months, snapshot.SourceAmount, snapshot.SourceCurrency, snapshot.Amount, snapshot.Currency, snapshot.Rate, snapshot.Provider)
	if err != nil {
		return Invoice{}, err
	}
	...
	return Invoice{ID: id, InvoiceNumber: number, WorkspaceID: workspaceID, PlanID: plan.ID, RequestedBy: userID, PlanName: plan.Name, PlanCode: plan.Code, InvoiceType: kind, BillingCycle: cycleDef.Name, PeriodMonths: cycleDef.Months, SourceAmountCents: snapshot.SourceAmount, SourceCurrency: snapshot.SourceCurrency, AmountCents: snapshot.Amount, Currency: snapshot.Currency, FXRate: snapshot.Rate, FXProvider: snapshot.Provider, FXMarkupBPS: snapshot.MarkupBPS, FXQuotedAt: snapshot.QuotedAt.Format(time.RFC3339), PeriodDays: legacyDays, Status: "pending", DueAt: due.Format(time.RFC3339)}, nil
}
...
func (s *Service) Settle(ctx context.Context, administratorID, invoiceID int64, status, note string) error {
	...
	if err = tx.QueryRowContext(ctx, `SELECT workspace_id,plan_id,status,invoice_type,period_months FROM billing_invoices WHERE id=? FOR UPDATE`, invoiceID).Scan(&workspaceID, &planID, &current, &invoiceType, &periodMonths); err != nil {
		return err
	}
	...
	if err = tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM payment_transactions WHERE invoice_id=? AND status IN ('created','pending')`, invoiceID).Scan(&activePayments); err != nil {
		return err
	}
	if activePayments > 0 {
		return errors.New("账单存在进行中的在线支付，请先等待支付结果")
	}
	if periodMonths < 1 {
		periodMonths = 1
	}
	if status == "void" {
		...
		_, err = tx.ExecContext(ctx, `UPDATE billing_invoices SET status='paid',paid_at=NOW(),paid_via='manual',admin_note=? WHERE id=?`, note, invoiceID)
...
func (s *Service) Invoices(ctx context.Context, status string, limit, offset int) ([]Invoice, error) {
	...
	query := `SELECT ` + invoiceSelect + ` FROM billing_invoices i JOIN plans p ON p.id=i.plan_id`
	args := []any{}
	if status != "" {
