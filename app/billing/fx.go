...
func (f *FXService) setting(ctx context.Context, key, fallback string) string {
	...
	if f.settings != nil {
		...
		if err != nil || !exists || strings.TrimSpace(stored) == "" {
			return fallback
		}
		value = stored
	} else {
		var encrypted bool
		if err := f.db.QueryRowContext(ctx, `SELECT setting_value,is_encrypted FROM system_settings WHERE setting_key=?`, key).Scan(&value, &encrypted); err != nil || encrypted || strings.TrimSpace(value) == "" {
			return fallback
		}
	}
	var decoded string
...
func (f *FXService) QuoteSettlement(ctx context.Context, sourceCurrency string, sourceAmount int64) (FXSnapshot, error) {
	...
	if err != nil || amount < 0 {
		return FXSnapshot{}, errors.New("结算金额换算失败")
	}
...
// referenceRate always derives a cross-currency quote from canonical USD-based
// rates. If 1 USD = S source units and 1 USD = T target units, then
// 1 source = T/S target. The frozen invoice snapshot stores this effective
// source->target rate, while cache/manual configuration remains USD-normalized.
func (f *FXService) referenceRate(ctx context.Context, provider, source, target string) (*big.Rat, time.Time, error) {
	sourcePerUSD, sourceObserved, err := f.usdRate(ctx, provider, source)
	if err != nil {
		return nil, time.Time{}, err
	}
	targetPerUSD, targetObserved, err := f.usdRate(ctx, provider, target)
	if err != nil {
		return nil, time.Time{}, err
	}
	if sourcePerUSD.Sign() <= 0 || targetPerUSD.Sign() <= 0 {
		return nil, time.Time{}, errors.New("汇率结果无效")
	}
	observed := sourceObserved
	if targetObserved.After(observed) {
		observed = targetObserved
	}
	if observed.IsZero() {
		observed = time.Now().UTC()
	}
	return new(big.Rat).Quo(targetPerUSD, sourcePerUSD), observed, nil
}

func (f *FXService) usdRate(ctx context.Context, provider, currency string) (*big.Rat, time.Time, error) {
	if currency == "USD" {
		return big.NewRat(1, 1), time.Now().UTC(), nil
	}
	if cached, observed, ok := f.cachedRate(ctx, "USD", currency, provider); ok {
		return cached, observed, nil
	}
	var rate *big.Rat
	...
	switch provider {
	case "manual":
		rate, err = f.manualUSDRate(ctx, currency)
		observed = time.Now().UTC()
	case "ecb":
		rate, observed, err = f.ecbUSDRate(ctx, currency)
	default:
	...
	if err = f.storeRate(ctx, "USD", currency, provider, rate, observed); err != nil {
		return nil, time.Time{}, err
	}
...
func (f *FXService) manualUSDRate(ctx context.Context, currency string) (*big.Rat, error) {
	raw := f.setting(ctx, "billing.fx.manual_rates", "{}")
	var rates map[string]string
	if json.Unmarshal([]byte(raw), &rates) != nil {
		return nil, errors.New("手动汇率配置格式无效")
	}
	pair := "USD/" + currency
	text := strings.TrimSpace(rates[pair])
	if text == "" {
		return nil, fmt.Errorf("未配置 USD/%s 手动汇率", currency)
	}
	rate, ok := new(big.Rat).SetString(text)
	if !ok || rate.Sign() <= 0 {
		return nil, fmt.Errorf("USD/%s 手动汇率无效", currency)
	}
	return rate, nil
}

func (f *FXService) ecbUSDRate(ctx context.Context, currency string) (*big.Rat, time.Time, error) {
	usdPerEUR, usdObserved, err := f.ecbRateAgainstEUR(ctx, "USD")
	if err != nil {
		return nil, time.Time{}, err
	}
	currencyPerEUR := big.NewRat(1, 1)
	currencyObserved := usdObserved
	if currency != "EUR" {
		currencyPerEUR, currencyObserved, err = f.ecbRateAgainstEUR(ctx, currency)
		if err != nil {
			return nil, time.Time{}, err
		}
	}
	observed := usdObserved
	if currencyObserved.After(observed) {
		observed = currencyObserved
	}
	return new(big.Rat).Quo(currencyPerEUR, usdPerEUR), observed, nil
}
...
func (f *FXService) ecbRateAgainstEUR(ctx context.Context, currency string) (*big.Rat, time.Time, error) {
	if !currencyCode.MatchString(currency) {
		return nil, time.Time{}, errors.New("汇率币种无效")
	}
	...
	if err != nil {
		return nil, time.Time{}, errors.New("暂时无法连接汇率服务")
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, time.Time{}, fmt.Errorf("汇率服务暂时不可用 (%d)", resp.StatusCode)
	}
	reader := csv.NewReader(io.LimitReader(resp.Body, 2<<20))
	header, err := reader.Read()
	if err != nil {
		return nil, time.Time{}, errors.New("汇率服务返回格式无效")
	}
	...
	if valueIndex < 0 {
		return nil, time.Time{}, errors.New("汇率服务缺少汇率值")
	}
	var rate *big.Rat
	var observed time.Time
	for {
		...
		if readErr != nil {
			return nil, time.Time{}, errors.New("汇率服务返回内容无效")
		}
		if valueIndex >= len(row) {
			continue
		}
		candidate, ok := new(big.Rat).SetString(strings.TrimSpace(row[valueIndex]))
		if !ok || candidate.Sign() <= 0 {
			continue
		}
		rate = candidate
		if timeIndex >= 0 && timeIndex < len(row) {
			if parsed, e := time.Parse("2006-01-02", strings.TrimSpace(row[timeIndex])); e == nil {
				observed = parsed.UTC()
			}
		}
	}
	if rate == nil {
		return nil, time.Time{}, errors.New("汇率服务没有返回可用报价")
	}
	if observed.IsZero() {
		observed = time.Now().UTC()
	}
