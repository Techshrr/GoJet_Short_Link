...
func (s *Service) ApplyPayment(ctx context.Context, invoiceID int64, provider, merchantOrder, providerReference string, amountCents int64, currency string) error {
	...
	var periodDays int
	if err = tx.QueryRowContext(ctx, `SELECT workspace_id,plan_id,invoice_type,status,amount_cents,currency,period_days FROM billing_invoices WHERE id=? FOR UPDATE`, invoiceID).Scan(&workspaceID, &planID, &invoiceType, &invoiceStatus, &invoiceAmount, &invoiceCurrency, &periodDays); err != nil {
		return err
	}
...
func (s *Service) InvoiceForPayment(ctx context.Context, userID, workspaceID, invoiceID int64) (Invoice, error) {
	...
	var out Invoice
	var due, created sql.NullTime
	var paid sql.NullTime
	err := s.db.QueryRowContext(ctx, `SELECT i.id,i.invoice_number,i.workspace_id,i.plan_id,i.requested_by,p.name,p.code,i.invoice_type,i.amount_cents,i.currency,i.status,i.due_at,i.paid_at,i.admin_note,i.created_at FROM billing_invoices i JOIN plans p ON p.id=i.plan_id WHERE i.id=? AND i.workspace_id=?`, invoiceID, workspaceID).Scan(&out.ID, &out.InvoiceNumber, &out.WorkspaceID, &out.PlanID, &out.RequestedBy, &out.PlanName, &out.PlanCode, &out.InvoiceType, &out.AmountCents, &out.Currency, &out.Status, &due, &paid, &out.AdminNote, &created)
	if err != nil {
		return Invoice{}, err
	}
	if due.Valid {
		out.DueAt = due.Time.UTC().Format("2006-01-02T15:04:05Z07:00")
	}
	if paid.Valid {
		out.PaidAt = paid.Time.UTC().Format("2006-01-02T15:04:05Z07:00")
	}
	if created.Valid {
		out.CreatedAt = created.Time.UTC().Format("2006-01-02T15:04:05Z07:00")
	}
	return out, nil
}
