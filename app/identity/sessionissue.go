...
func (s *Service) IssueSession(ctx context.Context, userID int64, ip, userAgent string) (User, string, error) {
    return s.IssueSessionWithTTL(ctx, userID, ip, userAgent, s.sessionTTL)
}

func (s *Service) IssueSessionWithTTL(ctx context.Context, userID int64, ip, userAgent string, ttl time.Duration) (User, string, error) {
    if ttl <= 0 || ttl > 30*24*time.Hour { return User{}, "", errors.New("登录会话有效期无效") }
    var user User
    ...
    if _, err = s.db.ExecContext(ctx, `INSERT INTO user_sessions(id,user_id,ip_address,user_agent,expires_at) VALUES(?,?,?,?,?)`, tokenHash, userID, nullable(ip), nullable(truncate(userAgent,500)), time.Now().Add(ttl)); err != nil { return User{}, "", err }
