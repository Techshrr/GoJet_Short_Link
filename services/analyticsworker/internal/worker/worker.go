...
func New(r *redis.Client, db *sql.DB, stream, group, consumer string, batch int64) *Worker {
	return &Worker{redis: r, db: db, stream: stream, group: group, consumer: consumer, batch: batch, claimIdle: 30 * time.Second, maxInvalidAttempts: 5, logger: observability.NewLogger("analyticsworker", observability.WriterFromEnvironment())}
}
