...
func (s *server) createQRCode(w http.ResponseWriter,r *http.Request){wid,err:=pathID(r,"id");var in struct{LinkID int64 `json:"link_id"`;Name,Foreground,Background string;Size int};if decode(w,r,&in)!=nil{return};if err!=nil{jsonResponse(w,400,map[string]string{"error":"工作区编号无效"});return};if err=s.requireQRLinkRiskAllow(r.Context(),wid,in.LinkID);err!=nil{jsonResponse(w,http.StatusUnprocessableEntity,map[string]string{"error":err.Error()});return};item,err:=s.resources.CreateQR(r.Context(),currentUser(r).ID,wid,in.LinkID,in.Name,in.Foreground,in.Background,in.Size);if err!=nil{jsonResponse(w,422,map[string]string{"error":err.Error()});return};jsonResponse(w,201,item)}
