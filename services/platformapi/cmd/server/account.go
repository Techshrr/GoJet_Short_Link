...
func (s *server) updateMe(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.identity.UpdateDisplayName(r.Context(), u.ID, in.DisplayName); err != nil {
		jsonResponse(w, http.StatusUnprocessableEntity, map[string]string{"error": err.Error()}); return
	}
	token,_ := sessionToken(r)
	updated, err := s.identity.Authenticate(r.Context(), token)
...
func (s *server) changeMyPassword(w http.ResponseWriter, r *http.Request) {
	var in struct { CurrentPassword string `json:"current_password"`; NewPassword string `json:"new_password"` }
	if decode(w, r, &in) != nil { return }
	user:=currentUser(r)
...
func (s *server) logoutUser(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.identity.RevokeToken(r.Context(), token); err != nil {
		jsonResponse(w, http.StatusServiceUnavailable, map[string]string{"error": "暂时无法退出登录"}); return
	}
	clearUserSessionCookies(w,r)
