...
func (s *server) deleteFileShare(w http.ResponseWriter, r *http.Request) {
	workspaceID, workspaceErr := pathID(r, "id")
	fileID, fileErr := pathID(r, "file")
	if workspaceErr != nil || fileErr != nil {
		jsonResponse(w, http.StatusBadRequest, map[string]string{"error": "编号无效"})
		return
	}
	...
	if raw := strings.TrimSpace(r.URL.Query().Get("retention_days")); raw != "" {
		...
		if err != nil || days < 0 || days > 30 {
			jsonResponse(w, http.StatusUnprocessableEntity, map[string]string{"error": "保留天数必须为 0 到 30"})
			return
		}
		retention = time.Duration(days) * 24 * time.Hour
	}
	if err := s.resources.DeleteFile(r.Context(), currentUser(r).ID, workspaceID, fileID, retention); err != nil {
		jsonResponse(w, http.StatusUnprocessableEntity, map[string]string{"error": "无法删除文件分享"})
		return
	}
	w.WriteHeader(http.StatusNoContent)
}
