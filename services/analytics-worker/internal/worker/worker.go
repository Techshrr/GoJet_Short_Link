...
const insertEvent = `INSERT IGNORE INTO analytics_events (stream_id,request_id,link_id,destination_id,occurred_at,visitor_hash,referer_url,referer_host,source_type,country,region,city,device,browser,operating_system,language,utm_source,utm_medium,utm_campaign,utm_content,utm_term,visit_type,is_bot) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
...
func New(r *redis.Client, db *sql.DB, stream, group, consumer string, batch int64) *Worker {
	return &Worker{redis: r, db: db, stream: stream, group: group, consumer: consumer, batch: batch, claimIdle: 30 * time.Second, maxInvalidAttempts: 5, logger: observability.NewLogger("analytics-worker", observability.WriterFromEnvironment())}
}
...
func (w *Worker) Run(ctx context.Context) error {
	...
	for {
		if err := w.consume(ctx); err != nil {
			...
			w.logger.Event(ctx, "error", "analytics.batch_failed", map[string]any{"error": err.Error(), "consumer": w.consumer})
			select {
			case <-ctx.Done():
				return nil
...
func (w *Worker) recordFailure(ctx context.Context, id string, cause error) int {
	...
	if err != nil {
		w.logger.Event(ctx, "error", "analytics.failure_record", map[string]any{"stream_id": id, "error": err.Error(), "original_error": cause.Error()})
		return 0
	}
	var attempts int
