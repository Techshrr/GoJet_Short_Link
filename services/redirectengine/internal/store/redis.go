...
func (s *RedisStore) command(ctx context.Context, args ...string) (any, error) {
	...
	if s.password != "" {
		...
		if s.username != "" { authArgs = []string{"AUTH", s.username, s.password} }
		if err = writeCommand(rw, authArgs...); err != nil { return nil, err }
		if _, err = readRESP(rw.Reader); err != nil { return nil, err }
	}
	if s.db != 0 {
		if err = writeCommand(rw, "SELECT", strconv.Itoa(s.db)); err != nil { return nil, err }
		if _, err = readRESP(rw.Reader); err != nil { return nil, err }
	}
	if err = writeCommand(rw, args...); err != nil { return nil, err }
	return readRESP(rw.Reader)
}
func writeCommand(w *bufio.ReadWriter, args ...string) error {
	if _, e := fmt.Fprintf(w, "*%d\r\n", len(args)); e != nil { return e }
	for _, a := range args { if _, e := fmt.Fprintf(w, "$%d\r\n%s\r\n", len(a), a); e != nil { return e } }
	return w.Flush()
}
func readRESP(r *bufio.Reader) (any, error) {
	...
	case ':': return strconv.ParseInt(line[1:], 10, 64)
	case '$':
		n, e := strconv.Atoi(line[1:]); if e != nil || n < 0 { return nil, e }
		b := make([]byte, n+2); _, e = io.ReadFull(r, b); return string(b[:n]), e
	default: return nil, fmt.Errorf("redis: unsupported response %q", line)
	}
}

func (s *RedisStore) SaveLink(ctx context.Context, l domain.Link) error {
	b, _ := json.Marshal(l)
	key := l.Code; if l.Domain != "" { key = l.Domain + "|" + l.Code }
	_, e := s.command(ctx, "SET", "gojet:link:"+key, string(b)); return e
}

func reachableTargets(l domain.Link) []string {
	targets := []string{l.Destination}
	for _, rule := range l.RoutingRules { targets = append(targets, rule.Destination) }
	for _, item := range l.Destinations { targets = append(targets, item.Destination) }
	return targets
}

func riskInterstitialURL(l domain.Link, decision string) string {
	base := strings.TrimRight(strings.TrimSpace(os.Getenv("PUBLIC_BASE_URL")), "/")
	if base == "" { return "" }
	reason := "review"; if decision == "block" { reason = "blocked" }
...
func enforceRiskDecision(l *domain.Link, raw any) {
	...
	if ok && decision == "allow" { return }
	if target := riskInterstitialURL(*l, decision); target != "" {
		l.Destination = target; l.StatusCode = 302; l.ExpiresAt = nil; l.PasswordHash = ""; l.MaxClicks = 0; l.OneTime = false; l.RoutingRules = nil; l.Destinations = nil; l.UTM = nil
		return
	}
	l.Active = false
}
...
func (s *RedisStore) FindLink(ctx context.Context, host, code string) (domain.Link, error) {
	var l domain.Link
	v, e := s.command(ctx, "GET", "gojet:link:"+host+"|"+code); if e != nil { return l, e }
	if v == nil { v, e = s.command(ctx, "GET", "gojet:link:"+code); if e != nil { return l, e } }
	if v == nil { return l, ErrNotFound }
	if e = json.Unmarshal([]byte(v.(string)), &l); e != nil { return l, e }
	// Only an exact ALLOW decision bound to the current target fingerprint can
	// pass through to the requested destination. Missing, stale, malformed, or
	// non-ALLOW state is handled by enforceRiskDecision as fail-closed.
	fingerprint := destinationkey.Fingerprint(reachableTargets(l))
	risk, e := s.command(ctx, "GET", "gojet:risk:"+l.ID+":"+fingerprint); if e != nil { return l, e }
...
return redis.call('XADD',KEYS[3],'*','link_id',ARGV[1],'destination_id',ARGV[2],'timestamp',ARGV[4],'visitor_hash',ARGV[3],'referer_url',ARGV[5],'referer_host',ARGV[6],'source_type',ARGV[7],'country',ARGV[8],'region',ARGV[9],'city',ARGV[10],'device',ARGV[11],'browser',ARGV[12],'operating_system',ARGV[13],'language',ARGV[14],'utm_source',ARGV[15],'utm_medium',ARGV[16],'utm_campaign',ARGV[17],'utm_content',ARGV[18],'utm_term',ARGV[19],'visit_type',ARGV[20],'is_bot',ARGV[21],'request_id',ARGV[25])`

func (s *RedisStore) RecordVisit(ctx context.Context, v domain.Visit) error {
	day := v.Timestamp.UTC().Format("2006-01-02"); month := v.Timestamp.UTC().Format("2006-01")
	...
	if e != nil && strings.Contains(e.Error(), "RATE_LIMITED") { return ErrRateLimited }
	if e != nil && strings.Contains(e.Error(), "LINK_EXHAUSTED") { return ErrExhausted }
	return e
}
func (s *RedisStore) Stats(ctx context.Context, id string) (domain.Stats, error) {
	get := func(key string) (int64, error) { v, e := s.command(ctx, "GET", key); if e != nil { return 0, e }; if v == nil { return 0, nil }; return strconv.ParseInt(v.(string), 10, 64) }
	clicks, e := get("gojet:clicks:" + id); if e != nil { return domain.Stats{}, e }
	today, e := get("gojet:daily:" + id + ":" + time.Now().UTC().Format("2006-01-02")); if e != nil { return domain.Stats{}, e }
	bots, e := get("gojet:bots:" + id); if e != nil { return domain.Stats{}, e }
	uv, e := s.command(ctx, "PFCOUNT", "gojet:visitors:"+id); if e != nil { return domain.Stats{}, e }
	readDims := func(prefix string, values []string) (map[string]int64, error) {
		out := map[string]int64{}
		for _, value := range values { n, e := get("gojet:" + prefix + ":" + id + ":" + value); if e != nil { return nil, e }; if n > 0 { out[value] = n } }
		return out, nil
	}
	sources, e := readDims("source", []string{"direct", "referer", "unknown"}); if e != nil { return domain.Stats{}, e }
	devices, e := readDims("device", []string{"desktop", "mobile", "tablet"}); if e != nil { return domain.Stats{}, e }
	browsers, e := readDims("browser", []string{"chrome", "edge", "firefox", "safari", "other"}); if e != nil { return domain.Stats{}, e }
...
func (s *RedisStore) Backlog(ctx context.Context) (int64, error) { v, e := s.command(ctx, "XLEN", "gojet:analytics:events"); if e != nil { return 0, e }; return v.(int64), nil }
