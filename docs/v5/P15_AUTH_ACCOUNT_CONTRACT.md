...
P15 completes the browser authentication and account-security surface on top of the existing Identity and social-provider services. It keeps the legacy Bearer path for non-browser clients while making the V5 browser contract server-side-session first.

## Auth shell
- `/login` and `/register` use the frozen 46/54 desktop Auth Shell and collapse to one column below 1024px.
- Login renders only providers returned by `/api/public/auth/providers`.
- Password login, OAuth handoff and OAuth registration all establish the same server-side `user_sessions` truth.
