...
独立 `analyticsreconciler` 按游标和有界批次遍历链接：
...
`tests/integration/redisanalytics.sh` 启动真实 `redis-server`，验证崩溃 Consumer 的 Pending 自动认领、计数补偿不降低并发实时值，以及死信重新投递的 Redis 级幂等性。该测试不使用内存模拟 Redis。
